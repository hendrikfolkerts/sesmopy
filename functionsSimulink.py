...
class functionsSimulink():
    ...
    def addComponents(self, objects, modelfile, modelname, interface):
        ...
        if interface == "FMI":  # for the FMI interface: in order to find the portnames of the imported FMU, the modelDescription.xml of the unzipped FMU needs to be parsed -> variable and import needs to be set
            ...
            fileobject.write("currentFMUfolders = {foldersFilesInfo(dirFlags).name};   %using the Matlab Mapping Toolbox: currentFMUfolders = extractfield(foldersFilesInfo(dirFlags),'name');\n")
            ...
            fileobject.write("    subDirFoldernames = {subDirFoldersFilesInfo(subDirFlags).name};     %using the Matlab Mapping Toolbox: subDirFoldernames = extractfield(subDirFoldersFilesInfo(subDirFlags),'name');\n")
            ...
            fileobject.write("startindOutputs = 1;   %in case the derivatives are part of the FMU outputs, find a new startindex (derivatives are first outputs of the FMU)\n")
            fileobject.write("if length(outportHandles) ~= length(outSignalNames)\n")
            fileobject.write("    startindOutputs = length(outportHandles) - length(outSignalNames) + 2;\n")
            fileobject.write("end\n")
            fileobject.write("for i = startindOutputs:length(outportHandles)\n")
            fileobject.write("    set_param(outportHandles(i), 'Name', outSignalNames{i - startindOutputs + 1});\n")
            fileobject.write("end\n")
